...
let svgPlot3 = (data) => {
	...
	//////adding to do small linked multiples
	// width = 150
	// height = 120
	...
	var g = svgheading.append('g') ////then append to global g - so now the div svg is appended
	...
	var g4 = svg4.append('g') ////then append to global g - so now the div svg is appended
		///note for multiple svgs you need a new g variable so you'd have g1, g2, etc
	...
	var g5 = svg5.append('g') ////then append to global g - so now the div svg is appended
		///note for multiple svgs you need a new g variable so you'd have g1, g2, etc
	...
	var g6 = svg6.append('g') ////then append to global g - so now the div svg is appended
		///note for multiple svgs you need a new g variable so you'd have g1, g2, etc
	...
	let headingmult = g.append('g') //// append text to global
		...
		.append('text')
		.style("fill", "#f5f4f9")
		.text('Curatorial Staff at NYC-Funded Museums, Grouped by Race and Ethnicity')
	...
	let ytext1 = g1.append("text")
		...
		.attr("y", width + margin.right / 1.2)
		.attr("x", 0 - (height / 2.9))
		.style("fill", "#FFD700")
		.text('museum curators => subset: those hired 2010-2015')
	...
	let line6_2area = g6.append("path")
		...
		.on('mouseenter', function (d) {
			...
			ytext6title1.transition()
				.duration(2000)
				.transition()
				.delay(3000)
				.ease(d3.easeQuadIn)
			// .delay(3000)
			// https://stackoverflow.com/questions/40765051/in-d3-v4-how-to-use-easebounce-propertly
			// d3.easeQuadIn(3000)
			// or css transitions but how to get changing text
		})
		.on('mouseout', function (d) {
			console.log(d)
			ytext6title1.text((d) => { return "curators"; })
