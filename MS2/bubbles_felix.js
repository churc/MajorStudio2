...
// 	console.log(margin.top)
    ...
    var svg = d3.select('#svg15')		  
        ...
        var modal = svg.append('g').attr("id", "modal")     /////does modal need width and height????    
          //     .attr("width", "250px") 
      			 // .attr("height", "40px")
              .attr("transform", "translate(" + margin.left + "," + margin.top + ")");
 ...
 let force = d3.forceSimulation(data)  /////gets concentric circles
  			  ...
  			  .force("center", d3.forceCenter().x(width/2).y(height/2))
  				.force("collision", d3.forceCollide().radius(function(d, i) {
  				    return radarea(d.count) + 3;
  				}));


var colors = d3.scaleSequential(d3.interpolateOrRd).domain([1, 14])
...
/////====== tooltip moving to one tooltip for each
// //tooltip for information
//         ////tooltip
//   let div = d3.select("g").append("div")
//     .attr("class", "tooltipcircles") ////best to use #tooltip div for styling
//     .style("opacity", 0)
//     .style("display", "none");
...
//enter
 ...
 var nodes = g.selectAll("g")
              ...
              .append("g")
                .on('mouseover', function(d,i) {
            		  console.log(i)
            		  d3.select(`#tooltip${i}`).style("display", "block").style("opacity", 1)
 ...
 var newtooltips = modal.selectAll("g")
  ...
  ////need this to keep width and height of modal and newtooltips      
