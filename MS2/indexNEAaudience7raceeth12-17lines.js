///NEA audience art museum attendance data 2012-17 race and ethnicity
            ...
            d3.json("NEA2012_2017audienceraceethlines.json").then((data) => {
                                 svgPlot24(data);
                    });
                //  console.log(data);
...
//         ////the above transform moves the whole chart away from the left and top of the browser
        	...
        	var margin = {top: 65, right: 90, bottom: 80, left: 90}; ////this works outside the svgPlot
	
	var width = 800 - margin.left - margin.right; // specify the width and give space around the chart
	var height = 500 - margin.top - margin.bottom; // specify the width and give space around the chart
	console.log(margin.top)
...
let line1 = g.append("path")
      ...
      d3.select("#tooltipline1")  //Update the tooltip position and value
            // .transition()
            // .duration(50)
        .style("left", (_x + margin.right*2 ) + "px")
        .style("top", (_y + margin.top*1.5 ) + "px")
            // .transition()
            // .ease(d3.easeLinear)
            // .duration(2000)
...
let line2 = g.append("path")
      ...
      d3.select("#tooltipline2")  //Update the tooltip position and value
        .style("left", (xPosition-width) + "px")
        .style("top", (yPosition-height) + "px")
        .select("#value")
...
// 				.append('text')
//     				.attr("class", "ytext2")
//                     .text('museum audience 2012-2017: 57.5 million adults')
...
//                     .attr('font-size', '65%'); 
 ...
 let heading = g.append('g')  //// append text to global
				.append('text')
                    ...
                    .text('Audience who visited an Art Museum or Gallery, 2012, 2017')
  ...
  let subhead1 = g.append('g')
				.append('text')  /////to site text you can always do negative values to be outside the chart
                    .attr("class", "audsubhead3")
                    .text('% trend lines by demographic subgroup')
                    .attr('x', margin.left/18)
