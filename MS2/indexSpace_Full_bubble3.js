...
d3.json("nationalityCountdatacontinent.JSON").then((data)=>{
 	                svgPlot15(data);
   ...
   var margin = {top: 10, right: 120, bottom: 60, left: 120}; ////this works outside the svgPlot
	
	var width = 1050 - margin.left - margin.right; // specify the width and give space around the chart
	var height = 1000 - margin.top - margin.bottom; // specify the width and give space around the chart
...
let heading = g.append('text')  //// append text to global
                .text('Brooklyn Museum Contemporary Collection: 10,253 artworks grouped by artist nationality')
                .attr('x', -margin.left)
                .attr('y', 50)
                .attr('font-size', '100%'); 
}
...
///////////////////========
 ...
 ////////=======previous code             
          ...
          //       d3.select("#tooltipcircles")  //Update the tooltip position and value
      ...
      //             .select("#valuecircles")
      //             .text(d.nationality + ' artists: ' + d.count + ' artworks')
               
              // d3.select(this).style("stroke", "#f742eb").style("stroke-weight", "5").style("stroke-opacity", 1)
               
      //           d3.select("#tooltipcircles").classed("hiddencircle", false);  //Show the tooltip
      //         })
